...
def _filter(rows: Sequence[ResearchObservation], *, htf_ob: bool | None = None,
    ...
    return [row for row in rows if (htf_ob is None or row.htf_ob == htf_ob) and (value_area is None or row.value_area == value_area) and (smc is None or row.smc == smc)]
...
def context_ablation(rows: Sequence[ResearchObservation]) -> dict[str, MetricSummary]:
    ...
    outcomes. True Base/Base+HTF-OB/Base+VA/Base+both trade-selection tests
    must replay the EA on the same bars with identical costs.
    """
    return {
        "base": summarize(rows),
...
def architecture_ablation(rows: Sequence[ResearchObservation]) -> dict[str, MetricSummary]:
    ...
    If variant-tagged replay rows are present, each result is a genuine
    variant result. Without tags, the fallback is explicitly candidate-level
    attribution and is not presented as a counterfactual backtest.
    """
    requested = ("midas_full", "midas_without_smc", "smc_only", "regime_plus_non_smc")
    by_variant = {name: [row for row in rows if row.variant == name] for name in requested}
...
def walk_forward_research(rows: Sequence[ResearchObservation], *, train_size: int, test_size: int,
                          learner: Callable[[Sequence[ResearchObservation]], object],
                          evaluator: Callable[[object, Sequence[ResearchObservation]], MetricSummary],
                          step: int | None = None) -> tuple[MetricSummary, ...]:
