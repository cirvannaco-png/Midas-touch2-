...
def test_pretrade_oms_router_child_tca_reconciliation_flow() -> None:
    ...
    risk = evaluate(
        ...
        daily_loss=0.0,
        spread_bps=2.0,
        limits=PreTradeLimits(1000.0, 5000.0, 2000.0, 100.0, 5.0),
    )
    assert risk.allowed
    ...
    children = ChildOrderExecutor(SimulatedVenue("v1", 99.9, 100.1, 100.0)).submit_children(stored)
