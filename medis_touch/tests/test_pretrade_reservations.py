...
def _reserve(book: RiskReservationBook, reservation_id: str, *, ttl_seconds: float | None = 30.0):
    return book.reserve(
        reservation_id, portfolio_notional=0, symbol_notionals={},
        requested_portfolio_notional=300, requested_symbol_notionals={"XAUUSD": 300},
        limits=_limits(), ttl_seconds=ttl_seconds,
    )
...
def test_reservation_rejects_non_finite_or_negative_inputs() -> None:
    ...
    with pytest.raises(ValueError):
        book.reserve(
            "bad-ttl", portfolio_notional=0, symbol_notionals={},
            requested_portfolio_notional=1, requested_symbol_notionals={"XAUUSD": 1}, limits=_limits(), ttl_seconds=0,
        )
