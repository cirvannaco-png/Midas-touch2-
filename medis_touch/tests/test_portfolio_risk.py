...
def test_portfolio_accounts_for_correlation():
    ...
    result = assess_portfolio(exposures, max_gross_notional=200, max_concentration_ratio=1.0, max_correlated_risk=95, correlation_matrix={("EURUSD", "GBPUSD"): 0.9})
    assert result.correlated_risk > 95
    assert not result.allowed
