...
async def get_copy_feed(
    ...
    """Return only currently authorized, valid signals to a subscriber."""
    try:
    ...
    result = await session.execute(
        ...
        ).order_by(Signal.received_at.desc()).limit(settings.COPY_FEED_MAX_SIGNALS)
    )
    ...
    return CopyFeedResponse(
        ...
        signals=[
            CopySignalItem(
                signal_id=s.signal_id, symbol=s.symbol, direction=s.direction,
