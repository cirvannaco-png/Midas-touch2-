...
async def receive_signal(
    ...
    db_signal = Signal(
        signal_id=payload.signal_id, symbol=payload.symbol, direction=payload.direction,
        entry=payload.entry, sl=payload.sl, tp1=payload.tp1, tp2=payload.tp2,
        confidence=payload.confidence, reasons=payload.reasons, timeframe=payload.timeframe,
        status=SignalStatus.PENDING, extra=payload.extra, regime=payload.regime,
        session=payload.session_tag, sweep_grade=payload.sweep_grade,
        htf_ob_aligned=payload.htf_ob_aligned, weight_version=payload.weight_version,
    )
    ...
    if await is_broadcast_paused(session) or await is_symbol_muted(session, payload.symbol):
        ...
        return SignalResponse(status="suppressed", signal_id=payload.signal_id, details="Broadcast paused or symbol muted - signal recorded, not sent to Telegram.")
    ...
    if status_signal in (SignalStatus.FAILED, SignalStatus.PERMANENTLY_FAILED):
        raise HTTPException(status_code=500, detail=f"Telegram sending failed. Signal saved with status '{status_signal.value}'.")
...
async def update_signal_lifecycle(signal_id: str, payload: LifecycleUpdateRequest, session: AsyncSession = Depends(get_session), _auth: bool = Depends(verify_api_key)):
    ...
    if db_signal.telegram_message_id is not None and new_status != SignalLifecycleStatus.VALID:
        original_text = format_signal_message({
            "signal_id": db_signal.signal_id, "symbol": db_signal.symbol, "direction": db_signal.direction,
            "entry": db_signal.entry, "sl": db_signal.sl, "tp1": db_signal.tp1, "tp2": db_signal.tp2,
            "timeframe": db_signal.timeframe, "confidence": db_signal.confidence,
            "reasons": db_signal.reasons, "extra": db_signal.extra,
        })
...
async def receive_trade_event(
    request: Request, payload: TradeEventRequest, session: AsyncSession = Depends(get_session),
    _auth: bool = Depends(verify_api_key), _rate: None = Depends(enforce_rate_limit),
):
    start_time = time.time()
    ...
    if not valid:
        raise HTTPException(status_code=400, detail={"event_id": payload.event_id, "errors": errors})
    db_event = TradeEvent(
        event_id=payload.event_id, trade_id=payload.trade_id, signal_id=payload.signal_id,
        symbol=payload.symbol, direction=payload.direction, event=payload.event,
        volume=payload.volume, price=payload.price, sl=payload.sl, tp1=payload.tp1, tp2=payload.tp2,
        profit=payload.profit, balance=payload.balance, equity=payload.equity, comment=payload.comment,
    ...
    try:
        ...
        return TradeEventResponse(status="duplicate", event_id=payload.event_id, trade_id=payload.trade_id, duplicate=True)
    ...
    if status_event in (TradeEventStatus.FAILED, TradeEventStatus.PERMANENTLY_FAILED):
        raise HTTPException(status_code=500, detail=f"Telegram sending failed. Trade event saved with status '{status_event.value}'.")
    return TradeEventResponse(status="sent", event_id=payload.event_id, trade_id=payload.trade_id, telegram_message_id=msg_id)
...
async def retry_failed_trade_events_core(session: AsyncSession) -> dict:
    ...
    result = await session.execute(select(TradeEvent).where((TradeEvent.status == TradeEventStatus.FAILED) | ((TradeEvent.status == TradeEventStatus.PENDING) & (TradeEvent.received_at < stale_before))).limit(5))
    ...
    for db_event in failed_events:
        event_dict = {
            "event_id": db_event.event_id, "trade_id": db_event.trade_id, "signal_id": db_event.signal_id,
            "symbol": db_event.symbol, "direction": db_event.direction, "event": db_event.event.value,
            "volume": db_event.volume, "price": db_event.price, "sl": db_event.sl, "tp1": db_event.tp1,
            "tp2": db_event.tp2, "profit": db_event.profit, "balance": db_event.balance,
            "equity": db_event.equity, "comment": db_event.comment,
        }
    ...
    return {"message": f"Processed {len(failed_events)} failed trade events. Retried {retried_count} successfully.", "remaining_failed": len(failed_events) - retried_count}


@router.post("/outcome", response_model=OutcomeResponse)
async def receive_outcome(payload: OutcomeRequest, session: AsyncSession = Depends(get_session), _auth: bool = Depends(verify_api_key), _rate: None = Depends(enforce_rate_limit)):
    log = logger.bind(signal_id=payload.signal_id)
...
async def get_config(symbol: str, session: AsyncSession = Depends(get_session), _auth: bool = Depends(verify_api_key)):
    latest = await session.scalar(select(ApprovedWeightVersion).order_by(ApprovedWeightVersion.approved_at.desc()).limit(1))
    if latest is None:
        return ConfigResponse(symbol=symbol, approved_weight_version=None, approved_at=None, params=None)
    return ConfigResponse(symbol=symbol, approved_weight_version=latest.weight_version, approved_at=latest.approved_at.isoformat() if latest.approved_at else None, params=None)
...
async def retry_failed_trade_events(session: AsyncSession = Depends(get_session), _auth: bool = Depends(verify_api_key), _rate: None = Depends(enforce_rate_limit)):
    return await retry_failed_trade_events_core(session)


async def retry_failed_signals_core(session: AsyncSession) -> dict:
    stale_before = datetime.now(timezone.utc) - timedelta(seconds=settings.PENDING_STALE_SECONDS)
    result = await session.execute(select(Signal).where((Signal.status == SignalStatus.FAILED) | ((Signal.status == SignalStatus.PENDING) & (Signal.received_at < stale_before))).limit(5))
    ...
    for db_signal in failed_signals:
        signal_dict = {
            "signal_id": db_signal.signal_id, "symbol": db_signal.symbol, "direction": db_signal.direction,
            "entry": db_signal.entry, "sl": db_signal.sl, "tp1": db_signal.tp1, "tp2": db_signal.tp2,
            "confidence": db_signal.confidence, "reasons": db_signal.reasons, "timeframe": db_signal.timeframe
        }
        ...
        await session.commit()
        await asyncio.sleep(0.5)
    return {"message": f"Processed {len(failed_signals)} failed signals. Retried {retried_count} successfully.", "remaining_failed": len(failed_signals) - retried_count}


@router.post("/retry-failed")
async def retry_failed_signals(session: AsyncSession = Depends(get_session), _auth: bool = Depends(verify_api_key), _rate: None = Depends(enforce_rate_limit)):
...
async def get_copy_feed(x_copy_key: str = Header(..., alias="X-Copy-Key"), session: AsyncSession = Depends(get_session)):
    """Authorize the subscriber through the fail-closed application service before reading the feed."""
    try:
        await authorize_copy_feed(session, x_copy_key)
