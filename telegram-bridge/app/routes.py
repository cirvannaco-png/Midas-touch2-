...
async def get_copy_feed(
    ...
    """Return the active feed only after the centralized authorization seam approves the subscriber."""
    try:
        await authorize_copy_feed(session, x_copy_key)
    except CopyFeedKeyUnknown as exc:
        raise HTTPException(status_code=401, detail=str(exc)) from exc
    except CopyFeedNotEntitled as exc:
    ...
    return CopyFeedResponse(copy_trading_enabled=True, signals=[
        CopySignalItem(
            signal_id=s.signal_id, symbol=s.symbol, direction=s.direction, entry=s.entry,
            sl=s.sl, tp1=s.tp1, tp2=s.tp2, confidence=s.confidence, timeframe=s.timeframe,
