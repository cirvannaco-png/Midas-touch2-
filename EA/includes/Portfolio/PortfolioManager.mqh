...
bool CPortfolioManager::AllowNewTrade(string symbol,double proposedRiskAmount,string &reasonOut)
  {
   ...
   string group=CorrelationGroup(symbol); double totalRisk=proposedRiskAmount; int symbolCount=0; int groupCount=0; bool unknown=false;
   for(int i=0;i<PositionsTotal();i++)
     {
      ...
      if(posSymbol==symbol) symbolCount++; if(CorrelationGroup(posSymbol)==group) groupCount++;
      double risk=OpenRiskAmount(ticket);
      if(risk<0.0) unknown=true; else totalRisk+=risk;
     }
   double equity=AccountInfoDouble(ACCOUNT_EQUITY); if(equity<=0.0){reasonOut="account equity unavailable/non-positive";return false;}
   double maxRiskAmount=equity*(m_maxPortfolioRiskPercent/100.0);
   if(symbolCount>=m_maxPositionsPerSymbol){reasonOut=StringFormat("%s already has %d open position(s) under this magic number (limit %d)",symbol,symbolCount,m_maxPositionsPerSymbol);return false;}
   if(groupCount>=m_maxPositionsPerGroup){reasonOut=StringFormat("correlation group %s already has %d open position(s) (limit %d)",group,groupCount,m_maxPositionsPerGroup);return false;}
   if(unknown){reasonOut="an existing open position under this magic number has uncomputable risk — refusing new exposure until its stop/risk can be verified";return false;}
   if(totalRisk>maxRiskAmount){reasonOut=StringFormat("adding this trade would bring total open risk to %.2f, above the %.2f%% portfolio cap (%.2f)",totalRisk,m_maxPortfolioRiskPercent,maxRiskAmount);return false;}
   return true;
  }
#endif
//+------------------------------------------------------------------+
