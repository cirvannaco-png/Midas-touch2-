...
string CPortfolioManager::CorrelationGroup(string symbol)
  {
   ...
   if(StringFind(symbol,"US30")>=0 || StringFind(symbol,"NAS100")>=0 || StringFind(symbol,"SPX")>=0 ||
      StringFind(symbol,"GER40")>=0 || StringFind(symbol,"UK100")>=0 || StringFind(symbol,"JPN225")>=0) return "INDICES";
   return "FX";
  }
//+------------------------------------------------------------------+
double CPortfolioManager::OpenRiskAmount(ulong ticket)
  {
   if(m_risk==NULL || !PositionSelectByTicket(ticket)) return 0.0;
   string symbol=PositionGetString(POSITION_SYMBOL);
   double entry=PositionGetDouble(POSITION_PRICE_OPEN);
   double sl=PositionGetDouble(POSITION_SL);
   double volume=PositionGetDouble(POSITION_VOLUME);
   if(sl<=0.0 || entry<=0.0 || volume<=0.0) return 0.0;
...
bool CPortfolioManager::AllowNewTrade(string symbol,double proposedRiskAmount,string &reasonOut)
  {
   ...
   if(m_maxPortfolioRiskPercent<=0.0) { reasonOut="portfolio risk cap is disabled/zero; refusing new exposure"; return false; }
   ...
   for(int i=0;i<PositionsTotal();i++)
     {
      ...
      if(risk<=0.0 && PositionGetDouble(POSITION_SL)<=0.0) anyUnknownRiskPosition=true;
      if(risk>0.0) totalRisk+=risk;
     }
   ...
   if(symbolCount>=m_maxPositionsPerSymbol)
     { reasonOut=StringFormat("%s already has %d open position(s) under this magic number (limit %d)",symbol,symbolCount,m_maxPositionsPerSymbol); return false; }
   if(groupCount>=m_maxPositionsPerGroup)
     { reasonOut=StringFormat("correlation group %s already has %d open position(s) (limit %d)",group,groupCount,m_maxPositionsPerGroup); return false; }
   if(anyUnknownRiskPosition)
     { reasonOut="an existing open position under this magic number has no stop loss set — portfolio risk cannot be verified"; return false; }
   if(totalRisk>maxRiskAmount)
     { reasonOut=StringFormat("adding this trade would bring total open risk to %.2f, above the %.2f%% portfolio cap (%.2f)",totalRisk,m_maxPortfolioRiskPercent,maxRiskAmount); return false; }
   return true;
  }
#endif
//+------------------------------------------------------------------+
