...
private:
   ...
   datetime          m_trackingStartBarTime[];
   int               m_count;
   int               m_maxBars;
   ...
   void              ResolveAmbiguousNoFill(int idx, string outcome, double exitPrice);
...
public:
   ...
   void              Update(CTFContext* entryCtx);
   int               ActiveCount() const { return m_count; }
   OutcomeStats      GetStats() const { return m_stats; }
   bool              GetFillState(datetime creation_time, long decisionId,
   ...
   void              ConfigureCalibration(bool enabled, int minSample = 30)
     { m_calibrationEnabled = enabled; m_calibration.Init(m_symbol, minSample); }
