...
private:
   ...
   bool m_calibrationEnabled; string m_symbol; ENUM_TIMEFRAMES m_entryTF; ENUM_FILL_POLICY m_fillPolicy; ENUM_TIMEFRAMES m_replayTF;
   OutcomeStats m_stats; double m_breakEvenAtR,m_partialAtR,m_partialFraction,m_trailATRMult,m_riskPercent;
   bool m_allowMinLotOverride,m_liveExecution; double m_commissionPerLot,m_spreadPoints,m_slippagePoints,m_decayHalfLifeBars; string m_weightVersion;
   ...
   double RValue(const PendingSetup &p) const
     {if(p.mgmtRiskDist<=0.0||p.lots<=0.0)return 0.0;double ts=SymbolInfoDouble(m_symbol,SYMBOL_TRADE_TICK_SIZE),tv=SymbolInfoDouble(m_symbol,SYMBOL_TRADE_TICK_VALUE);if(ts<=0.0||tv<=0.0)return 0.0;double oneR=p.mgmtRiskDist*(tv/ts)*p.lots;return oneR>0.0?p.realizedPnL/oneR:0.0;}
...
public:
   ...
   bool MarkClosed(long id,double exitPrice,datetime t,double netPnl,double commission,double swap,double fee,bool stillOpen,string outcome)
     {int i=Find(id);if(i<0)return false;PendingSetup p=m_pending[i];if(!p.filled)return false;p.realizedPnL+=netPnl;p.totalCommission+=commission;p.totalSlippageCost+=fee;if(stillOpen){m_pending[i]=p;return true;}m_pending[i]=p;Finalize(i,outcome,exitPrice);return true;}
