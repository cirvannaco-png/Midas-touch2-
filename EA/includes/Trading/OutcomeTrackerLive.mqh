...
//| Broker-authoritative outcome lifecycle.                         |
//+------------------------------------------------------------------+
...
class COutcomeTrackerLive
  {
private:
   PendingSetup m_pending[]; int m_count; int m_maxBars;
   CSignalLogger* m_logger; CSignalPublisher* m_publisher; CCalibrationEngine m_calibration;
   bool m_calibrationEnabled; string m_symbol; ENUM_TIMEFRAMES m_entryTF; ENUM_FILL_POLICY m_fillPolicy;
   ENUM_TIMEFRAMES m_replayTF; OutcomeStats m_stats;
   double m_breakEvenAtR,m_partialAtR,m_partialFraction,m_trailATRMult,m_riskPercent;
   bool m_allowMinLotOverride,m_liveExecution;
   ...
   double RValue(const PendingSetup &p) const
     {
      if(p.mgmtRiskDist<=0.0||p.lots<=0.0)return 0.0;
      double ts=SymbolInfoDouble(m_symbol,SYMBOL_TRADE_TICK_SIZE),tv=SymbolInfoDouble(m_symbol,SYMBOL_TRADE_TICK_VALUE);
      if(ts<=0.0||tv<=0.0)return 0.0; double oneR=p.mgmtRiskDist*(tv/ts)*p.lots; return oneR>0.0?p.realizedPnL/oneR:0.0;
     }
...
public:
   ...
   void AddSetup(TradeSetup &setup,long decisionId=-1)
     {
      if(!setup.active||decisionId<=0||Find(decisionId)>=0)return;PendingSetup p;ZeroMemory(p);p.setup=setup;p.decisionId=decisionId;p.entryRef=ResolveExecutionEntry(setup);p.riskDist=MathAbs(p.entryRef-setup.stop_loss);p.sizingEntryPrice=p.entryRef;p.mgmtRiskDist=p.riskDist;p.mfePrice=p.entryRef;p.maePrice=p.entryRef;p.currentSL=setup.stop_loss;p.confidenceAtSignal=setup.confidence;p.confidenceDecayed=setup.confidence;int n=m_count++;ArrayResize(m_pending,m_count);m_pending[n]=p;
     }
   bool MarkExecuted(long id,double fill,datetime t,double volume)
     {
      int i=Find(id);if(i<0||fill<=0.0)return false;PendingSetup p=m_pending[i];if(p.filled)return true;p.filled=true;p.fillPrice=fill;p.entryFillPrice=fill;p.entryRef=fill;p.fillTime=t;p.barsToFill=0;p.lots=volume;p.remainingLots=volume;p.riskDist=MathAbs(fill-p.setup.stop_loss);p.mgmtRiskDist=p.riskDist;p.mfePrice=fill;p.maePrice=fill;p.currentSL=p.setup.stop_loss;m_pending[i]=p;return true;
     }
   bool MarkClosed(long id,double exitPrice,datetime t,double pnl,double commission,double swap,double fee,bool stillOpen,string outcome)
     {
      int i=Find(id);if(i<0)return false;PendingSetup p=m_pending[i];if(!p.filled)return false;p.realizedPnL+=pnl;p.totalCommission+=commission;p.totalCommission+=swap+p.totalCommission*0.0;p.totalSlippageCost+=fee;if(stillOpen){m_pending[i]=p;return true;}m_pending[i]=p;Finalize(i,outcome,exitPrice);return true;
     }
   void Update(CTFContext* ctx)
     {
      ...
      for(int i=m_count-1;i>=0;i--)
        {
         PendingSetup p=m_pending[i];int shift=iBarShift(m_symbol,m_entryTF,p.setup.creation_time,false);datetime creationBar=shift>=0?iTime(m_symbol,m_entryTF,shift):p.setup.creation_time;
         ...
         p=m_pending[i];int fshift=iBarShift(m_symbol,m_entryTF,p.fillTime,false);datetime fillBar=fshift>=0?iTime(m_symbol,m_entryTF,fshift):p.fillTime;if(bar.time<=fillBar)continue;if(p.lastBarTime==bar.time)continue;
         p.barsElapsed++;p.lastBarTime=bar.time;bool buy=p.setup.type==ORDER_TYPE_BUY;if(buy){if(bar.high>p.mfePrice)p.mfePrice=bar.high;if(bar.low<p.maePrice)p.maePrice=bar.low;}else{if(bar.low<p.mfePrice)p.mfePrice=bar.low;if(bar.high>p.maePrice)p.maePrice=bar.high;}if(m_decayHalfLifeBars>0){p.decayBars=p.barsElapsed;p.confidenceDecayed=p.confidenceAtSignal*MathPow(0.5,(double)p.decayBars/m_decayHalfLifeBars);}m_pending[i]=p;if(p.barsElapsed>=m_maxBars)Finalize(i,"timeout",bar.close);
        }
     }
