...
private:
   ...
   string Bucket(double value,double a,double b,double c) const
     {
      if(value<a)return "0";
      if(value<b)return "1";
      if(value<c)return "2";
   ...
   string EnvironmentKey(const SetupReasons &r) const
     {
      ...
      return EnumToString(r.regime)+"|"+
             ...
             "|V"+EnumToString(r.va_zone)+"|P"+EnumToString(r.phase)+"|M"+structure;
     }

   static double WilsonZ() { return 1.959963984540054; }

   double WilsonLower(int wins,int n) const
     {
      if(n<=0)return 0.0;
      double z=WilsonZ(),p=(double)wins/n,den=1.0+z*z/n;
      double center=p+z*z/(2.0*n);
      double spread=z*MathSqrt((p*(1.0-p)/n)+(z*z/(4.0*n*n)));
      return (center-spread)/den;
     }

   double WilsonUpper(int wins,int n) const
     {
      if(n<=0)return 0.0;
      double z=WilsonZ(),p=(double)wins/n,den=1.0+z*z/n;
      double center=p+z*z/(2.0*n);
   ...
   void Save() const
     {
      ...
      int h=FileOpen(m_filename,FILE_CSV|FILE_WRITE|FILE_ANSI,',');
      if(h==INVALID_HANDLE)return;
      FileWrite(h,"timestamp","environment_key","strategy","realized_r","mae_r","mfe_r","duration","counted");
      for(int i=0;i<ArraySize(m_records);i++)
         FileWrite(h,(long)m_records[i].timestamp,m_records[i].environment_key,m_records[i].strategy,
                   m_records[i].realized_r,m_records[i].mae_r,m_records[i].mfe_r,m_records[i].duration,
                   m_records[i].counted?1:0);
      FileClose(h);
     }
   ...
   void Load()
     {
      ArrayFree(m_records);
      if(StringLen(m_filename)==0||!FileIsExist(m_filename))return;
      int h=FileOpen(m_filename,FILE_CSV|FILE_READ|FILE_SHARE_READ|FILE_ANSI,',');
      if(h==INVALID_HANDLE)return;
      for(int c=0;c<8&&!FileIsEnding(h);c++)FileReadString(h);
      while(!FileIsEnding(h))
        {
         string ts=FileReadString(h);if(StringLen(ts)==0)break;
         EnvironmentMemoryRecord r;ZeroMemory(r);
         r.timestamp=(datetime)StringToInteger(ts);
         r.environment_key=FileReadString(h);
         r.strategy=(int)StringToInteger(FileReadString(h));
         r.realized_r=StringToDouble(FileReadString(h));
         r.mae_r=StringToDouble(FileReadString(h));
         r.mfe_r=StringToDouble(FileReadString(h));
         r.duration=(int)StringToInteger(FileReadString(h));
         r.counted=(StringToInteger(FileReadString(h))!=0);
...
public:
   ...
   void RecordOutcome(const SetupReasons &r,ENUM_SELECTED_STRATEGY strategy,double realizedR,
                      double maeR,double mfeR,int duration,datetime timestamp)
     {
      if(strategy==STRATEGY_NONE)return;
      EnvironmentMemoryRecord rec;ZeroMemory(rec);
      rec.timestamp=timestamp;
      rec.environment_key=EnvironmentKey(r);
      rec.strategy=(int)strategy;
      rec.realized_r=realizedR;
      rec.mae_r=maeR;
      rec.mfe_r=mfeR;
      rec.duration=MathMax(0,duration);
      rec.counted=true;
      int n=ArraySize(m_records);ArrayResize(m_records,n+1);m_records[n]=rec;
      Save();
     }
   ...
   bool GetEvidence(const SetupReasons &r,ENUM_SELECTED_STRATEGY strategy,EnvironmentMemoryEvidence &out) const
     {
      ZeroMemory(out);
      if(strategy==STRATEGY_NONE)return false;
      string key=EnvironmentKey(r);
      double cumulative=0.0,peak=0.0;
      double sumR=0.0,sumMAE=0.0,sumMFE=0.0,sumDuration=0.0,grossProfit=0.0,grossLoss=0.0;
      ...
      for(int i=0;i<ArraySize(m_records);i++)
        {
         ...
         if(rec.realized_r>0.0){wins++;grossProfit+=rec.realized_r;}
         else if(rec.realized_r<0.0){losses++;grossLoss+=-rec.realized_r;}
         else scratches++;
        }
      out.trades=n;out.wins=wins;out.losses=losses;out.scratches=scratches;out.sample_size=wins+losses;
      out.win_rate=out.sample_size>0?(double)wins/out.sample_size:0.0;
      out.avg_r=n>0?sumR/n:0.0;
      out.profit_factor=grossLoss>0.0?grossProfit/grossLoss:(grossProfit>0.0?-1.0:0.0);
      out.avg_mae_r=n>0?sumMAE/n:0.0;
      out.avg_mfe_r=n>0?sumMFE/n:0.0;
      out.avg_duration=n>0?sumDuration/n:0.0;
      out.wilson_low=WilsonLower(wins,out.sample_size);
      out.wilson_high=WilsonUpper(wins,out.sample_size);
      if(out.sample_size<m_minSample)
        {
