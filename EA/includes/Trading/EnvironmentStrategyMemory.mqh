...
class CEnvironmentStrategyMemory
  {
private:
   EnvironmentMemoryRecord m_records[]; string m_filename; int m_minSample; int m_degradedMinSample; double m_bonus; double m_penalty; bool m_persistent;
   ...
   string EnvironmentKey(const SetupReasons &r) const
     {double spreadAtr=0.0;if(r.atr_value>0.0&&r.point_size>0.0)spreadAtr=r.spread_points*r.point_size/r.atr_value;string structure=EnumToString(r.sweep_grade)+"/"+IntegerToString((int)r.breakout_class)+"/"+IntegerToString((int)r.reversion_class)+"/"+IntegerToString((int)r.keylevel_reaction);string trendBucket=Bucket(MathAbs(r.trend_strength),0.25,0.60,0.85);return EnumToString(r.regime)+"|"+EnumToString(r.vol_regime)+"|T"+trendBucket+"|L"+IntegerToString(r.liquidity_bucket)+"|N"+EnumToString(r.news_risk)+"|S"+EnumToString(r.session)+"|SP"+Bucket(spreadAtr,0.02,0.05,0.10)+"|A"+EnumToString(r.vol_regime)+"|H"+(r.htf_ob_confluence?"1":"0")+"/"+IntegerToString((int)r.htf_ob_state)+"|V"+EnumToString(r.va_zone)+"|P"+EnumToString(r.phase)+"|M"+structure;}
