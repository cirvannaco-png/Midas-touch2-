...
int OnInit()
  {
   ...
   g_scoring.ConfigureInducement(InpImpulseLookbackBars,InpImpulseATRMult,InpImpulseBodyRatio,InpEqualTolATR,InpMaxLegExtend,
                                 InpRequirePremiumDiscount,InpRequireDistributionPhase,InpPhaseRangeLookback,InpPhaseCompressionATRMult);
   ...
   g_scoring.ConfigureSessionFilter(InpUseSessionFilter,InpAllowTokyoSession,InpAllowLondonSession,InpAllowNewYorkSession,InpAllowLondonNYOverlap);
   g_scoring.ConfigureSweepQuality(InpRequireMinSweepGrade,(ENUM_SWEEP_GRADE)InpMinSweepGrade,InpRequireFreshSetup,InpMaxBarsSinceBOS);
   ...
   g_scoring.ConfigureStrategyDiagnostics(InpMomentumBreakoutRecencyBars,InpBreakoutLiqOverlapBars,InpBreakoutExtensionLookbackBars,InpBreakoutExhaustionATRMult,InpMomentumLookbackBars);
   g_scoring.ConfigureMeanReversionDiagnostics(InpReversionMinStretchATR,InpReversionSRZoneATRTolerance,InpReversionWickRejectionRatio,InpReversionLiqRecencyBars,InpReversionTrendConflictRecencyBars,InpReversionTrendConflictMinStrength);
   g_scoring.ConfigureKeyLevelDiagnostics(InpKeyLevelLookbackBars,InpKeyLevelSearchATRMax,InpKeyLevelTouchToleranceATRMult,InpKeyLevelAbsorptionMinTouches,InpKeyLevelWickRejectionRatio,InpKeyLevelRoundStep);
   ...
   // Outcome simulation is explicitly tied to the EA execution timeframe.
   // InpFVGTF remains analysis-only and can never silently become the
   // execution/backtest clock.
   g_tracker.Init(&g_logger,_Symbol,_Period,InpMaxTrackingBars,InpFillPolicy,InpReplayTF);
   g_tracker.ConfigureSimulation(InpRiskPercentPerTrade,InpAllowMinLotOverride,InpBreakEvenAtR,InpPartialAtR,InpPartialFraction,InpTrailATRMult,
   ...
   g_router.Init(_Symbol,InpEnableExecution,InpEnableSignals,InpMinConfidenceExecute,InpMinConfidenceSignal,InpFullRiskConfidence,InpMaxSpreadPoints);
   ...
   g_positions.Init(&g_orders,&g_broker,InpBreakEvenAtR,InpPartialAtR,InpPartialFraction,InpTrailATRMult,0,0.0,0.0,5,&g_chartCtx.swings);
   ...
   if(InpUseNewsFilter) g_newsFilter.Load(InpNewsFilterFile,InpNewsMinutesBefore,InpNewsMinutesAfter);
   g_scoring.ConfigureNewsAwareness(GetPointer(g_newsFilter),InpNewsWarnMinutesBefore,InpNewsWarnMinutesAfter,InpNewsWarnMultiplier);
   g_recovery.Init(&g_store,&g_orders,InpMagicNumber,_Symbol);
   int restoredCount=g_recovery.Recover();
   if(restoredCount>0) PrintFormat("MedisTouch EA: recovery restored %d live trade(s) from a prior session.",restoredCount);
   TradeDecisionRecord priorDecisions[];
   int priorCount=g_store.LoadAll(priorDecisions);
   long maxId=0;
   for(int i=0;i<priorCount;i++) if(priorDecisions[i].decision_id>maxId) maxId=priorDecisions[i].decision_id;
...
void OnDeinit(const int reason)
  {
   if(StringLen(InpConfigSyncEndpoint)>0) EventKillTimer();
   g_publisher.Deinit();
...
void CheckSignalLifecycle(double currentAtr)
  {
   ...
   bool haveState=g_tracker.GetFillState(g_lifecycleCreationTime,g_lifecycleDecisionId,filled,fillPrice,fillTime,barsToFill);
   if(!haveState){g_lifecycleDecisionId=0;return;}
   if(filled){g_lifecycleDecisionId=0;return;}
   ...
   if(barsSinceCreation>=InpSignalExpiryBars)
     {
      if(g_lifecycleStatus!="expired")
        {
         g_publisher.PublishStatusUpdate(g_lifecycleDecisionId,"expired",StringFormat("Unfilled for %d execution bars (max %d)",barsSinceCreation,InpSignalExpiryBars));
         g_lifecycleStatus="expired";
        }
      g_lifecycleDecisionId=0; return;
     }
   ...
   if(oppositeConfidence>=InpInvalidateOpposingConfidence)
     {
      if(g_lifecycleStatus!="invalidated")
        {
         g_publisher.PublishStatusUpdate(g_lifecycleDecisionId,"invalidated",StringFormat("Opposing setup confidence reached %.0f",oppositeConfidence));
      ...
      if(wentWrongWay && driftATR>=InpSignalStaleChaseATR)
        {
         if(g_lifecycleStatus!="stale")
           { g_publisher.PublishStatusUpdate(g_lifecycleDecisionId,"stale",StringFormat("Price drifted %.2f ATR past entry, unfilled",driftATR)); g_lifecycleStatus="stale"; }
        }
      else if(g_lifecycleStatus=="stale" && driftATR<InpSignalStaleChaseATR*0.5)
        { g_publisher.PublishStatusUpdate(g_lifecycleDecisionId,"valid","Price returned toward entry zone"); g_lifecycleStatus="valid"; }
     }
  }
...
void OnTick()
  {
   ...
   string haltReason;
   if(g_riskGuard.IsHardHalted(haltReason)) return;
   if(g_riskGuard.IsDailyLossLimitHit(haltReason)) return;
   string newsReason;
   if(InpUseNewsFilter && g_newsFilter.IsLocked(newsReason)) return;
   ...
   // Advance every existing outcome on every execution-timeframe bar.
   // This must not depend on whether a new setup was generated this bar.
   // New setups are added only after this call, so their creation bar can
   // never be retrospectively processed.
   ...
   TradeSetup chosen; ZeroMemory(chosen);
   ...
   if(buySetup.active && sellSetup.active)
     {
      if(confDelta>=InpMinDirectionalAdvantage)
        { if(g_risk.ValidateSetup(buySetup,InpMinRiskReward,InpMaxSLDistanceATR,currentAtr)) chosen=buySetup; }
      else if(-confDelta>=InpMinDirectionalAdvantage)
        { if(g_risk.ValidateSetup(sellSetup,InpMinRiskReward,InpMaxSLDistanceATR,currentAtr)) chosen=sellSetup; }
     }
   ...
   if(InpLogSignals)
     { ENUM_TREND_STATE t=g_trendCtx.trend.GetCurrentTrend(); g_logger.LogSetup(chosen,_Symbol,InpFVGTF,EnumToString(t)); }

   TradeDecisionRecord decision=g_router.Decide(chosen);
   if(InpTrackOutcomes) g_tracker.AddSetup(chosen,decision.decision_id);
   ...
   if(decision.action==POLICY_EXECUTE_ONLY || decision.action==POLICY_EXECUTE_AND_SIGNAL)
     {
      ...
      double lots=g_risk.CalculateLotSize(_Symbol,InpRiskPercentPerTrade,entry,chosen.stop_loss,decision.reduce_risk,InpAllowMinLotOverride,exceededRiskBudget,g_riskGuard.SizeMultiplier());
      if(lots<=0)
         PrintFormat("MedisTouch EA: decision #%d skipped — risk is below broker minimum lot for %s.",decision.decision_id,_Symbol);
      else
        {
         ...
         if(!g_portfolio.AllowNewTrade(_Symbol,proposedRisk,blockReason))
         ...
         else
           {
            ...
            if(g_orders.Submit(decision,lots,InpUseMarketOrders,maxDeviation,ticketOut)) g_store.SaveExecution(decision.decision_id,lots,ticketOut);
            else g_monitor.NotifyBrokerReject();
           }
        }
     }
...
void OnTradeTransaction(const MqlTradeTransaction &trans,const MqlTradeRequest &request,const MqlTradeResult &result)
  {
   ...
   if(g_orders.MarkFilledFromPending(orderTicket,positionTicket,fillPrice))
      PrintFormat("MedisTouch EA: pending order #%d filled as position #%d at %.5f.",orderTicket,positionTicket,fillPrice);
  }
//+------------------------------------------------------------------+
